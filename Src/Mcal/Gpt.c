...
void Gpt_Init(const Gpt_ConfigType * ConfigPtr)
{
    ...
    if( (ConfigPtr -> Gpt_ChannelNum)%2 == 0) /* we are in Timer A */
    {
        ...
        if(ConfigPtr -> Gpt_ChannelRunningMode == GPT_ONE_SHOT) // step 3 in initialization of one shot / periodic mode
        {
        ...
        else 
        {
          ...
          *(volatile uint32 *)((volatile uint8 *)GPT_Ptr + GPT_GPTMTAMR_REG_OFFSET) |= 0x02; // select periodic mode
				...
				*(volatile uint32 *)((volatile uint8 *)GPT_Ptr + GPT_GPTMTAPR_REG_OFFSET) = ConfigPtr -> Gpt_prescaler ;
    ...
    else /* Timer B */ 
    {
        ...
        else 
        {
          ...
          *(volatile uint32 *)((volatile uint8 *)GPT_Ptr + GPT_GPTMTBMR_REG_OFFSET) |= 0x02; // select periodic mode 
				...
				*(volatile uint32 *)((volatile uint8 *)GPT_Ptr + GPT_GPTMTBPR_REG_OFFSET) = ConfigPtr -> Gpt_prescaler ;
